...
def get_dataloader(
    ...
    if batch_size > 1:
        if distributed:
            batch_sampler_phase = DistributedTrajBatchSampler(
                dataset_phase,
        ...
        loader_phase = DataLoader(
            ...
            batch_sampler=batch_sampler_phase,
    ...
    if distributed:
        sampler_phase = DistributedSampler(
            dataset_phase,
            num_replicas=world_size,
            rank=rank,
            shuffle=shuffle,
    ...
    loader_phase = DataLoader(
        ...
        batch_size=1,
        shuffle=(sampler_phase is None and shuffle),
        drop_last=drop_last,
        pin_memory=pin_memory,
        num_workers=num_workers,
        persistent_workers=(num_workers > 0),
...
class DistributedTrajBatchSampler(Sampler):
    r"""DDP-compatible mini-batch sampler for trajectory datasets.

    It first builds the same batch list on all ranks and then shards by rank.
    """
    ...
    def __iter__(self):
        ...
        padded_num_batches = int(math.ceil(num_batches / self.num_replicas) * self.num_replicas)
        if padded_num_batches > num_batches:
            all_batches = all_batches + all_batches[:padded_num_batches - num_batches]
        rank_batches = all_batches[self.rank:padded_num_batches:self.num_replicas]
        return iter(rank_batches)

    def __len__(self):
        num_batches = len(self._build_all_batches())
        if num_batches == 0:
            return 0
        if self.even_divisible:
            return int(math.ceil(num_batches / self.num_replicas))
        if self.rank >= num_batches:
            return 0
        return ((num_batches - 1 - self.rank) // self.num_replicas) + 1
